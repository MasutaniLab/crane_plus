...
CallbackReturn CranePlusHardware::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  auto period = rclcpp::Duration(0, 0);
  read(time, period);
...
return_type CranePlusHardware::read(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
...
return_type CranePlusHardware::write(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  if (communication_timeout()) {
    if (!timeout_has_printed_) {
      RCLCPP_ERROR(
